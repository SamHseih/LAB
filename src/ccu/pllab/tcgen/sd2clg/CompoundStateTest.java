...
public class CompoundStateTest {// *****This is like-main
	...
	public static void main(String[] args)
			throws ParserConfigurationException, SAXException, IOException, TemplateException, ModelAccessException,
			ParseException, org.apache.commons.cli.ParseException, JSONException, EclipseException {
		...
		/*
		 * st = new SDXML2SD().convert(new File("./CoffeeMachineSD1.uml"), // new
		 * File("./coffeemachine1.uml")); new File("./coffeeMachineClass.uml"));
		 */
		...
		/*
		 * CLGGraph clg = new SDXML2SD().converts( // new
		 * File("./CoffeeMachineSD1.uml"), new File("./coffeeMachineClass.uml"), new
		 * File("./coffeemachine1.uml"));
		 */
		...
		// String content="";
		/*
		 * content += clg.getStartNode().getSuccessor().get(0).toGenImg2(); for(Object
		 * key:clg.getConstraintCollection().keySet()) {
		 * content+=clg.getConstraintCollection().get(key).toGenImg2(); } for(int
		 * i=0;i<clg.getConnectionNode().size();i++) {
		 * content+=clg.getConnectionNode().get(i).toGenImg2(); }
		 * DataWriter.writeInfo(content,
		 * //clg.getStartNode().getSuccessor().get(0).toGenImg2(), "CLG", "dot",
		 * "../examples/ACLG","CLG"); new ProcessBuilder("dot", "-Tpng",
		 * "../examples/ACLG/CLG/CLG.dot", "-o",
		 * "../examples/ACLG/CLG/CLG.png").start();
		 */
		// List<ccu.pllab.tcgen.ast.Constraint>OCLCons=parseOCL(new File("./A.ocl"), new
		// File("./Aclass.uml"));
		/*
		 * List<ccu.pllab.tcgen.ast.Constraint>OCLCons=parseOCL(new
		 * File("./coffeemachine.ocl"), new File("./CoffeeMachine.uml"));
		 * 
		 * CoverageCriterionManager coverman = new CoverageCriterionManager();
		 * Main.criterion=Criterion.dc; //coverman.init(clg, Main.criterion,OCLCons);
		 * coverman.init(gtclggraph, Main.criterion,OCLCons);
		 */

		/*
		 * List<TestDataClassLevel> resulttd = new ArrayList<TestDataClassLevel>();
		 * resulttd = coverman.genClassLevelTestSuite(); TestScriptGeneratorClassLevel
		 * testscri = new TestScriptGeneratorClassLevel();
