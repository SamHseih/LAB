...
public class ExecuteCLP {
	...
	public ArrayList query(String target, String sdName) {
		...
		try {
			...
			/* 執行query */
			String target_predicate = String.format(sdName.substring(0, 1).toLowerCase() + sdName.substring(1)
					+ "Preamble(%s, FuncSeq, ArgSeq), write_exdr(eclipse_to_java ,FuncSeq), write_exdr(eclipse_to_java ,ArgSeq).",
					target);
