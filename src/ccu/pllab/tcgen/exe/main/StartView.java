	...
	public void setListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fileChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
					...
					if (selectedFile.getName().contains("ocl")) {
						...
						JOptionPane.showMessageDialog(null, selectedFile.getName() + "Error Import",
								"Please Select File", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		};
		...
		this.start_actionListener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					 ...
					 * ast.getAbstractSyntaxTree().toGraphViz(); Main.ast =
					 * ast.getAbstractSyntaxTree(); clg = new AST2CLG(Main.ast); CLG2Path path = new
					 * CLG2Path(clg.getCLGGraph(), clg.getInvCLG(), ast.getSymbolTable(), classUml,
					 * ast.getAutMap()); JOptionPane.showMessageDialog(null, "Execution Succeed",
					 * "Result", JOptionPane.INFORMATION_MESSAGE); Main.invCLP = "";
					 */
		...
		this.windowAdapter = new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				int result = JOptionPane.showConfirmDialog(startView, "Are you sure to end the program?", "Exit",
