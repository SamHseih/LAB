...
//		this.window.add(ast);
		...
		/*
		 * this.exit = new JButton("End"); this.exit.setBounds(90,130,100,40);
		 * this.exit.addActionListener(this.exit_actionListener); this.window.add(exit);
		 */
	...
	public void setListener() {
		...
		/*
		 * this.exit_actionListener=new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { int
		 * result=JOptionPane.showConfirmDialog(window, "確定要結束程式嗎?", "確認訊息",
		 * JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE); if
		 * (result==JOptionPane.YES_OPTION) {window.dispose();} } };
		 */
		this.windowAdapter = new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				int result = JOptionPane.showConfirmDialog(window, "Are you sure to end the program?", "Exit",
