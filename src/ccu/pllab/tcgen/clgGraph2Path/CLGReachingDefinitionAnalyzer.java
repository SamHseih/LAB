...
public class CLGReachingDefinitionAnalyzer extends CLGGraph {
		...
		if (clgnode.getLeftOperand() instanceof CLGOperatorNode) {
			...
			if (strop.equals("=")) {
				if (!def.contains(((CLGVariableNode) clgnode.getLeftOperand()).getName())) {
					...
					if (defstr.contains("[")) {
						String useStr = defstr.substring(defstr.indexOf("[") + 1, defstr.indexOf("]"));
						if (useStr.contains("+"))
	...
	public ArrayList<DUP> dupGenerate(CLGGraph clg, String classname, String methodname) {
			...
			if (clgcon instanceof CLGMethodInvocationNode) {
				...
				for (int j = 0; j < this.hashtable.size(); j++) {
					if (clgmethod.getMethodName().equals(this.hashtable.get(j).get("method_name"))) {
						...
						// 20200503 dai
						// alldef415 =
						// (ArrayList<String>)this.hashtable.get(j).get("define");
						def415.add((ArrayList<String>) this.hashtable.get(j).get("define"));
				...
				ht.put("define", def);
				ht.put("use", use);
				ht.put("nodenum", consnum.get(i));
				ht.put("class_name", classname);
				ht.put("method_name", methodname);
				ht.put("clg", clg);
		...
		// 2020-02-dai
		System.out.println("def415 = " + def415);
		...
		// 20200218 dai
		System.out.println("kill315: " + kill315);
		...
		for (int j = 0; j < def415.size(); j++) {
			finalgen315.add(inte); // 2020-05-04 clg 限制試Block
		...
		// ------------------build pre node---------------------------------
		// 20200503 dai -> prenode315 後面的code 好像沒用
		// ArrayList<ArrayList<Integer>> prenode315 = new
		...
		for (int i = 0; i < finalgen315.size(); i++) {
			regout315.add(saveregout315);
			regin315.add(saveregin315);
		}
		// ----------------end initial in out------------------------
