...
public class FacadeHook {
	...
	public void when_gen_ast_graph(Model context, FacadeConfig config) throws IOException {
			...
			for (Operation op : clazz.getOwnedOperations()) {
				...
				if (method_nodes.size() > 0) {
					File ast_dot_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "ast.dot", uml2Operation.getOwner().getName(),
									uml2Operation.getName()));
					File ast_dot_img_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "ast.png", uml2Operation.getOwner().getName(),
									uml2Operation.getName()));
					...
					new ProcessBuilder("dot", "-Tpng", ast_dot_file.getAbsolutePath(), "-o",
							ast_dot_img_file.getAbsolutePath()).start();
		...
		new ProcessBuilder("dot", "-Tpng", ast_dot_file.getAbsolutePath(), "-o", ast_dot_img_file.getAbsolutePath())
				.start();
	}

	public void when_gen_clg_graph(Model context, FacadeConfig config, List<JSONObject> config_list, TypeTable typeTab)
		...
		for (UML2Class clazz : context.getClasses()) {
			for (Operation op : clazz.getOwnedOperations()) {
				...
				for (JSONObject it_config : config_list) {
					if (it_config.getString("target_class")
							.equals(String.format("%s.%s", context.getPackageName(), clazz.getName()))
							&& it_config.getString("target_method").equals(op.getName())) {
						method_config = it_config;
						break;
					}
				}

				CLGGraph clg_graph = uml2Operation
						.getCLG(FacadeConfig.parsePathCoverage(method_config.optString("path_coverage", "node")));
				if (clg_graph != null) {
					/* start ocl2clp */
					String methodCLP = ((CLGStartNode) clg_graph.getStartNode()).CLG2MethodCLP(typeTab);
					System.out.println(methodCLP);
					/* end ocl2clp */
					File clg_dot_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "clg.dot", uml2Operation.getOwner().getName(),
									uml2Operation.getName()));
					File clg_dot_img_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "clg.png", uml2Operation.getOwner().getName(),
									uml2Operation.getName()));
					String dotContent = clg_graph.graphDraw();
					allDotContent += dotContent;
					writeFile(dotContent, clg_dot_file);
					new ProcessBuilder("dot", "-Tpng", clg_dot_file.getAbsolutePath(), "-o",
							clg_dot_img_file.getAbsolutePath()).start();
				}
			}
		}
		...
		new ProcessBuilder("dot", "-Tpng", clg_dot_file.getAbsolutePath(), "-o", clg_dot_img_file.getAbsolutePath())
				.start();
		...
		for (UML2Class clazz : context.getClasses()) {
			...
			for (Operation op : clazz.getOwnedOperations()) {
				...
				for (ccu.pllab.tcgen.ast.Constraint pre : uml2Operation.getPreConstraints()) {
					...
					nodes.add(new_pre);
					File ast_dot_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "invalid_ast.dot",
									uml2Operation.getOwner().getName(), uml2Operation.getName()));
					File ast_dot_img_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "invalid_ast.png",
									uml2Operation.getOwner().getName(), uml2Operation.getName()));
					if (!ast_dot_img_file.getParentFile().exists()) {
						ast_dot_img_file.getParentFile().mkdirs();
					}
	...
	public void when_gen_invalid_clg_graph(Model context, FacadeConfig config, List<JSONObject> config_list)
			throws IOException, JSONException {
		String allDotContent = "";
		for (UML2Class clazz : context.getClasses()) {
			for (Operation op : clazz.getOwnedOperations()) {
				...
				CLGGraph clg_graph_list = uml2Operation
						.getInvalidCLG(FacadeConfig.parsePathCoverage(method_config.optString("path_coverage", "dcc")));
				if (clg_graph_list != null) {
					File clg_dot_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "invalid_clg.dot",
									uml2Operation.getOwner().getName(), uml2Operation.getName()));
					File clg_dot_img_file = new File(config.getOutputFolder().getFile(),
							String.format("%s%s" + File.separatorChar + "invalid_clg.png",
									uml2Operation.getOwner().getName(), uml2Operation.getName()));
					String dotContent = "";
					dotContent = clg_graph_list.graphDraw();
					/*
					 * for (CLGNode clgs : clg_graph_list) { dotContent +=
					 * GraphViz.generateGraph(clgs); }
					 */
					allDotContent += dotContent;
					writeFile(dotContent, clg_dot_file);
					new ProcessBuilder("dot", "-Tpng", clg_dot_file.getAbsolutePath(), "-o",
	...
	public void when_gen_path_graph(Model context, FacadeConfig config, List<Path> csp_path_list) throws IOException {
		for (Path p : csp_path_list) {
			...
			File path_dot_img_file = new File(config.getOutputFolder().getFile(),
					p.getPredicateName().replaceFirst("tcgen_\\d+_", "") + File.separatorChar + p.getPredicateName()
							+ ".png");
			writeFile(GraphViz.generateGraph(p), path_dot_file);
			new ProcessBuilder("dot", "-Tpng", path_dot_file.getAbsolutePath(), "-o",
					path_dot_img_file.getAbsolutePath()).start();
		}
		...
		new ProcessBuilder("dot", "-Tpng", path_dot_file.getAbsolutePath(), "-o", path_dot_img_file.getAbsolutePath())
