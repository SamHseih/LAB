...
public class SingleCDParser {
	 ...
	 * 以下為Parse主要及所需method
	 * 
	 * 1. 將類別圖轉成dom樹狀結構 : init(), initRef() 2. 分析並取得類別圖裡的資訊 : Parse()
	 */
	...
	/*
	 * // 把參考用的uml圖list 轉成dom private void initRef( ArrayList<File> rList ) { try {
	 * this.ref = new ArrayList<Document>();
	 * 
	 * // 定義XML DOM parser解析器 DocumentBuilderFactory dbFactory =
	 * DocumentBuilderFactory.newInstance(); // 建立DOM document DocumentBuilder
	 * dBuilder = dbFactory.newDocumentBuilder();
	 * 
	 * for(int i = 0 ; i < rList.size() ; i++ ) { doc =
	 * dBuilder.parse(rList.get(i)); // 引入xml doc.getDocumentElement().normalize();
	 * // 針對xml文檔的元素做normalize this.ref.add(doc) ; } // for
	 * 
	 * } catch (Exception e) { e.printStackTrace(); } } // initRef()
	 */
	// NO.1
	// =====================================================================================
	...
	public void Parse(Document doc)
		...
		for (int i = 0; i < nList.getLength(); i++) {
			...
			VariableType this_class_type = null;
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				this_class_type = newVariableType(c); // 20210309 dai 新增
				...
				if (pList.getLength() != 0)
					property_List = new ArrayList<VariableInfo>();

				// 取得此class裡的各個property的type & name
				for (int j = 0; j < pList.getLength(); j++) {
					Element child = (Element) pList.item(j);
					VariableInfo p = new VariableInfo(getVarType(child, c.getID(), c.getName()),
							child.getAttribute("name"), child.getAttribute("xmi:id"), getAttrVisibility(child),
							c.getName());
					if (child.getElementsByTagName("lowerValue").item(0) != null) {
						...
						if (temp.hasAttribute("value"))
							p.setLowerValue(temp.getAttribute("value"));
						else
							p.setLowerValue("0");
					} // if
				...
				UserDefinedType userDef = new UserDefinedType(e.getAttribute("name"),e.getAttribute("xmi:id"));
				Main.typeTable.add(userDef);

				// ---------- Operation ----------
				NodeList oList = e.getElementsByTagName("ownedOperation"); // 取得此class的所有operation
				if (oList.getLength() != 0)
					operation_List = new ArrayList<OperationInfo>();
				...
				for (int k = 0; k < oList.getLength(); k++) {
					...
					System.out.println(
							"CD Parser / Class Name: " + method.getName() + ", ParaNum: " + parameter.getLength());
					...
					// 取得method所有參數的name, type 以及 return type
						...
						for (int p_i = 0; p_i < parameter.getLength(); p_i++) {
							...
							VariableInfo p = new VariableInfo(getVarType(eParameter, c.getID(), c.getName()),
									eParameter.getAttribute("name"), eParameter.getAttribute("xmi:id"), "",
									c.getName());
			...
			if (this_class_type instanceof UserDefinedType)
				((UserDefinedType) this_class_type).setClassInfo(c);
	...
	// 取得property的type: 自建立的型別, 基本型別, 陣列
	private VariableType getVarType(Element e, String classID, String className)
			throws ParserConfigurationException, SAXException, IOException, TransformerException {
		/*
		...
		if (et.getAttribute("xmi:type").equals("uml:PrimitiveType")) {
			...
			if (!low.equals("1") && !up.equals("1")) {
				//uses comma to split every dimension of the array
				String[] indexList = up.split(",");
				ArrayType arrayTypeBuild = new ArrayType(new VoidType(), "0");
				//create multi-dimension arrayType
				for(int i=indexList.length-1; i>=0; i--) {
					if(i==indexList.length-1)
						arrayTypeBuild = new ArrayType(Main.typeTable.get(s, s), indexList[i]);
					else
						arrayTypeBuild = new ArrayType(arrayTypeBuild, indexList[i]);
				}
				Main.typeTable.add(arrayTypeBuild);
				return arrayTypeBuild;
			}
			/* 判斷是否是array */

			else
				return Main.typeTable.get(s, s);
		} // if
		else if (et.getAttribute("xmi:type").equals("uml:Parameter")){
			
			String s = et.getAttribute("type");
		...
		// 自定義型別 (ArrayList, String, UserDefinedType)
			...
			if (s.equals("ArrayList")) { // ArrayList的話要另外抓裡面的element type
				...
				// comment_type = comment_type.substring( comment_type.indexOf("<")+1,
				// comment_type.length()) ;
				tempType = createArrayListType(comment_type);
				Main.typeTable.add(tempType);
			} // if
			else {
				tempType = Main.typeTable.get(s, s);
			} // else
	...
	private ArrayListType createArrayListType(String s)
			throws ParserConfigurationException, SAXException, IOException, TransformerException {
	...
	private UserDefinedType createUserDefinedType(String s)
			throws ParserConfigurationException, SAXException, IOException, TransformerException {
	...
	private String getAttrVisibility(Element e) {
		if (e.hasAttribute("visibility"))
			return e.getAttribute("visibility");
		else
			return "public";
	}
	...
	/*
	 * private void changeRef() { for(int i =0; i < this.getClassList().size();i++)
	 * { now = this.getClassList().get(i); if( searchRefUserDefinedType(now.getID(),
	 * now.getName()) == null ); else
	 * 
	 * 
	 * for( int i = 0 ; i < classList.size() ; i++ ) { ClassInfo c
	 * =classList.get(i); for(int j= 0; c.getProperties()!= null && j <
	 * c.getProperties().size();j++ ) { VariableInfo p = c.getProperties().get(j);
	 * if(p.getType() instanceof UserDefinedType) { if(
	 * searchRefUserDefinedType(now.getID(), now.getName()) == null ); }
	 * 
	 * }
	 * 
	 * for(int k= 0; c.getOperations()!= null && k < c.getOperations().size();k++ )
	 * { OperationInfo o = c.getOperations().get(k);
	 * s=s+o.getReturnType().getType().toString() +" " + o.getName() + " " +
	 * o.getID() + " " + o.getVisibility()+ " " + o.getClassName();
	 * s=s+"Parameter: "; for(int index = 0 ;o.getParameter()!= null && index <
	 * o.getParameter().size();index++) { VariableInfo p =
	 * o.getParameter().get(index); s=s+p.getType().toString() + " " + p.getName() +
	 * " " + p.getID() + " " + p.getVisibility()+ " " + p.getClassName(); }
	 * s=s+"\n"; } s=s+"\n================================"; } }
	 */
	/*
	 * // 2020/05 以下函式可能不會用到
	 ...
	 * ------------------------------------------------------------
	 * 
	 * // 將變數的型別與自己建立的class的id做對應, 把id改成class的name // 如果型別是陣列要把在name後面加上[] ****待改
	 * public void changeTypeStr() {
	 * 
	 * for (int i= 0 ; i < this.classList.size();i++){ // class local variable
	 * for(int j = 0 ; this.classList.get(i).getProperties() != null && j <
	 * this.classList.get(i).getProperties().size();j++) { VariableInfo p =
	 * this.classList.get(i).getProperties().get(j);
	 ...
	 * 
	 * 
	 * // class method for(int j = 0 ; this.classList.get(i).getOperations() != null
	 * && j < this.classList.get(i).getOperations().size();j++){ OperationInfo
	 * o=this.classList.get(i).getOperations().get(j); // class method parameter
	 * for(int para_i = 0 ; o.getParameter() != null && para_i <
	 * o.getParameter().size();para_i++) { VariableInfo parameter =
	 * o.getParameter().get(para_i);
	 * parameter.setType(typeIDtoName(parameter.getType()));
	 * 
	 * // if(parameter.getSize() != null)
	 * parameter.setType(parameter.getType()+"[]"); // size非空即為陣列 } // for
	 * 
	 * // return type VariableInfo rt = o.getReturnType() ;
	 * if(o.getReturnType()!=null) rt.setType(typeIDtoName(rt.getType()));
	 * 
	 * // *** if(o.getReturnType().getSize() != null) o.setType(p.getType()+"[]");
	 * // size非空即為陣列 }//for operation } } // changeTypeStr()
	 * 
	 * 
	 * // 如果是自定義的型別, 在類別圖裡type是以class ID的形式記錄 // 所以要去將id轉換成相對應的型別名稱 //
	 * 被changeTypeStr()呼叫 private String typeIDtoName( String type ) { //
	 * System.out.println(classList.size()); String s = type ; for(int k = 0 ; k <
	 * this.classList.size();k++ ) { // System.out.println((k+1) +
	 * this.classList.get(k).getID());
	 * 
	 * if (type.equals(this.classList.get(k).getID())) { // System.out.println(type
	 * + "**"+this.classList.get(k).getID() + " "+ this.classList.get(k).getName());
	 * s = this.classList.get(k).getName(); } } //for
	 * 
	 * return s; } // typeIDtoName()
	 * 
	 * 
	 * public void parseRef() { NodeList list =
	 * doc.getElementsByTagName("packageImport"); Node node = list.item(4); //
	 * 第i個class if (node.getNodeType() == Node.ELEMENT_NODE) { Element e = (Element)
	 * node; NodeList child = e.getElementsByTagName("importedPackage"); Element ch
	 * = (Element) child.item(0); System.out.println(ch.getAttribute("href")); File
	 * f = new File(ch.getAttribute("href")); if( f.exists() )
	 * System.out.println("O"); else System.out.println("X"); }
	 * 
	 * }
	 */
	// 於Parse()中所用到的method
	...
	public String printParseClassInfo(ClassInfo c) {
		...
		try {
			...
			for (int j = 0; c.getProperties() != null && j < c.getProperties().size(); j++) {
				...
				s = s + p.getType().toString() + " " + p.getName() + " " + p.getID() + " " + p.getVisibility() + " "
						+ p.getClassName();
			}
			...
			for (int k = 0; c.getOperations() != null && k < c.getOperations().size(); k++) {
				OperationInfo o = c.getOperations().get(k);
				s = s + o.getReturnType().getType().toString() + " " + o.getName() + " " + o.getID() + " "
						+ o.getVisibility() + " " + o.getClassName();
				s = s + "Parameter: ";
	...
	private VariableType newVariableType(ClassInfo theClass) {

		if (theClass.getName().equals("int") || theClass.getName().equals("Integer")) {
			return new IntType();
		} else if (theClass.getName().equals("string") || theClass.getName().equals("String")) {
			return new StringType();
		} else if (theClass.getName().equals("char") || theClass.getName().equals("Char")) {
			return new CharType();
		} else if (theClass.getName().equals("boolean") || theClass.getName().equals("Boolean")) {
			return new BooleanType();
		} else if (theClass.getName().equals("real") || theClass.getName().equals("Real")) {
			return new DoubleType(); //自動
		} else if (theClass.getName().equals("float") || theClass.getName().equals("Float")) {
			return new FloatType();
		} else if (theClass.getName().equals("double") || theClass.getName().equals("Double")) {
			return new DoubleType();
		} else {
			return new UserDefinedType(theClass.getName(), theClass.getID());
		}
	}
