...
public class IfExp extends ASTNode {
	...
	public CLGNode toCLG(Criterion criterion) {
		...
		ASTNode astNotNode = new OperationCallExp(this.getConstraint(), astnotCondNode, "not",
	...
	@Override
	public IfExp clone() {
		IfExp n = new IfExp(this.getConstraint(), this.getConditionExp().clone(), this.getThenExp().clone(),
				this.getElseExp().clone());
		return n;
	}
	...
	public String getEntirePredicate(Map<String, String> templateArgs) {
		...
		tpl.add("node_identifier", this.getId());
		tpl.add("condition",
				this.getConditionExp().getPredicateName(new HashMap<String, String>()).replaceAll("\\(.*\\)", ""));
		tpl.add("then_predicate",
				this.getThenExp().getPredicateName(new HashMap<String, String>()).replaceAll("\\(.*\\)", ""));
		tpl.add("else_predicate",
				this.getElseExp().getPredicateName(new HashMap<String, String>()).replaceAll("\\(.*\\)", ""));
		for (Map.Entry<String, String> entry : templateArgs.entrySet()) {
			tpl.add(entry.getKey(), entry.getValue());
		}
		return tpl.render();
	...
	public CLGGraph OCL2CLG() {
		...
		if (ifconstraint.getSourceExp() instanceof OperationCallExp
				&& !ifconstraint.getSourceExp().toString().contains("()")) {
		...
		/************/
		if (ifconstraint.getParameterExps().get(0) instanceof OperationCallExp
				&& !ifconstraint.getParameterExps().get(0).toString().contains("()")) {
			...
			Right = new CLGObjectNode(ifconstraint.getParameterExps().get(0).toString());
			op = ifconstraint.getPropertyName().toString();
			if (op.equals("=")) {
				OperatiorConstraint = new CLGOperatorNode("==");
			} else {
			...
			Right = new CLGObjectNode(ifconstraint.getParameterExps().toString().substring(1,
					ifconstraint.getParameterExps().toString().length() - 1));
			op = ifconstraint.getPropertyName().toString();
			if (op.equals("=")) {
				OperatiorConstraint = new CLGOperatorNode("==");
			} else {
