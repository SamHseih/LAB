...
public class IfExp extends AbstractSyntaxTreeNode {
	...
	public void addVariableType(TypeTable typeTable, String methodName) {
		this.conditionExp.addVariableType(typeTable, methodName);
		this.thenExp.addVariableType(typeTable, methodName);
		if (this.elseExp != null)
			this.elseExp.addVariableType(typeTable, methodName);
	}
	...
	public CLGConstraint AST2CLG(SymbolTable sym, String methodName, boolean isCondition) {
		CLGConstraint condition = this.conditionExp.AST2CLG(sym, methodName, true);
		CLGConstraint notcondition = this.Demongan(condition);
		CLGConstraint then = this.thenExp.AST2CLG(sym, methodName, isCondition);
		if (this.elseExp != null) {
			CLGConstraint elseExp = this.elseExp.AST2CLG(sym, methodName, isCondition);
...
//	}
	...
	public CLGConstraint Demongan(CLGConstraint Demonganconstraint) {
		...
		finaltree.setType(((CLGOperatorNode) Demonganconstraint).getType());
