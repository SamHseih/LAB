...
public class OperationContext extends AbstractSyntaxTreeNode {
	...
	private VariableType returnType = new VoidType();
//	private int preNum = 0; //20210726  好像沒用到
//	private int postNum = 0;//20210726  好像沒用到
	...
	// OCL規格， operationContext
	public void setReturnType(String returnType) {
		this.returnType = Main.typeTable.get(returnType, null);
		;
	}
...
//	}
	...
	public String AST2CLP(String attribute, String argument) {
		String clp = ":-lib(ic).\n:-lib(timeout).\n" + this.methodName
				+ "(Obj_pre,Arg_pre,Obj_post,Arg_post,Result):-\n";
		...
		if (attribute.length() > 0)
			for (String obj : object)
				clp += obj.toUpperCase().charAt(0) + obj.substring(1) + "=" + obj.toUpperCase().charAt(0)
						+ obj.substring(1) + "_pre,\n";
		if (argument.length() > 0)
			for (String var : arg)
				clp += var.toUpperCase().charAt(0) + var.substring(1) + "=" + var.toUpperCase().charAt(0)
						+ var.substring(1) + "_pre,\n";
		clp = clp.substring(0, clp.length() - 2) + ".\n\n";
		for (StereoType stereo : stereoType)
			clp += stereo.AST2CLP(attribute, argument);
		/*
		 * if(attribute!=null) { clp+="["; String obj="",objPre=""; for(VariableToken
		 * attr:attribute) obj+=attr.getVariableName()+",";
		 * 
		 * objPre=obj.replaceAll(",", "_pre,"); clp+=objPre+obj; clp=clp.substring(0,
		 * clp.length()-1); clp+="]::-32768..32767,\n";
		 * clp+="Obj_pre=["+objPre.substring(0, objPre.length()-1)+"],\n"; clp+="Obj_" +
		 * "post=["+obj.substring(0, obj.length()-1)+"],\n"; }
		 * if(method.getArgument().size()>0) {
		 * 
		 * String argPost="",argPre=""; for(VariableToken arg:method.getArgument())
		 * argPost+=arg.getVariableName()+","; argPre=argPost.replace(",", "_pre,");
		 * clp+="["+argPre+argPost.substring(0,argPost.length()-1)+
		 * "]::-32768..32767,\n"; clp+="Arg_pre=["+argPre.substring(0,
		 * argPre.length()-1)+"],\n"; clp+="Arg_post=["+argPost.substring(0,
		 * argPost.length()-1)+"],\n"; }
		 * clp+="method(Obj_pre,Arg_pre,Obj_post,Arg_post,Result).\n"; for(StereoType
		 * stereo:stereoType) clp+=stereo.AST2CLP(attribute, method);
		 */
		return clp;
	}
