...
public class TestScriptGeneratorClassLevel {
	...
	// ----
	public String genTestCase(List<TestDataClassLevel> data, String packagePosition, String packageName,
				...
				if (i1 == 0) { // constructor
					...
					if (!inprearg.isEmpty()) {
						for (int argp = 0; argp < inprearg.size(); argp++) {
							...
							if (inprearg.size() != 1 && argp != (inprearg.size() - 1))
								str += ",";
						}
					}
					str += ");\n";
					...
					for (int statep = 0; statep < posts1.size(); statep++) {
						if (posts1.get(statep) instanceof Integer) {
							 ...
							 * "\t\tassertTrue("+"o"+count+".equals(coffee.getmoney()));\n"; str +=
							 * "\t\tassertTrue("+"o"+count+".equals(coffee.get"+att.get(statep)+"()));\n";
							 */count++;
							str += "\t\tassertEquals(" + posts1.get(statep) + ",coffee.get" + att.get(statep)
									+ "());\n";
						} else if (posts1.get(statep) instanceof String) {
							// str +=
							// "\t\tassertTrue(\""+posts1.get(statep)+"\".equals(coffee.getmoney()));\n";
							// str +=
							// "\t\tassertTrue(\""+posts1.get(statep)+"\".equals(coffee.get"+att.get(statep)+"()));\n";
							str += "\t\tassertEquals(\"" + posts1.get(statep) + "\",coffee.get" + att.get(statep)
									+ "());\n";
						}
					}
					...
					if (inret.isEmpty()) {
						...
						for (int inc = 0; inc < inret.size(); inc++) {
							...
							if (!inprearg.isEmpty()) {
								/*
								 * if(inprearg.get(0) instanceof Integer){ str +=
								 * "coffee."+methodname.get(i1)+"("+inprearg.get(0)+")));\n"; } else
								 * if(inprearg.get(0) instanceof String){ str +=
								 * "coffee."+methodname.get(i1)+"(\""+inprearg.get(0)+"\")));\n"; }
								 */
								for (int argp = 0; argp < inprearg.size(); argp++) {
									if (inprearg.get(argp) instanceof Integer) {
					...
					for (int statep = 0; statep < inposts.size(); statep++) {
						if (inposts.get(statep) instanceof String) {
							...
							// "\t\tassertTrue(\""+posts1.get(statep)+"\".equals(coffee.getmoney()));\n";
							// str +=
							// "\t\tassertTrue(\""+inposts.get(statep)+"\".equals(coffee.get"+att.get(statep)+"()));\n";
